...
#region Members
  ...
  /// </summary>
  private void RunScript(List<Point3d> Points, int Degree, int Divisions, int U, int V, ref object Mesh, ref object Srf, ref object SourceSrf, ref object TargetSrf, ref object Orientation)
  {
    
    //riktig oppdatert 03.06
    List<double> length = new List<double>();
    List <NurbsCurve> edgecrv = new List<NurbsCurve>();
    DataTree<NurbsCurve> edgecrvtree = new DataTree<NurbsCurve>();
    ...
    // NurbsCurve crv = NurbsCurve.Create(false, Degree, PointsSource[1] PointsMid[1] PointsTarget[1]);
    DataTree<Point3d> clst = new DataTree<Point3d>(); //make ordered list to make curve
    //List <NurbsCurve> nrbs = new List<NurbsCurve>();
    Point3d ps;
    for (int i = 0;i < 4;i++)
    {
      GH_Path pthnr = new GH_Path(i);
      for (int j = i;j < Points.Count();j += 4) //if more midpoints
      {
        ps = Points[j];
        clst.Add(ps, pthnr);
      }
      edgecrv.Add(NurbsCurve.Create(false, Degree, clst.Branch(i)));
      length.Add(edgecrv[i].GetLength());
      //edgecrvtree.Add(NurbsCurve.Create(false, Degree, clst.Branch(i)), new GH_Path(i));
      vrttree.AddRange(edgecrv[i].DivideEquidistant(length[i] / Divisions), new GH_Path(i));
      vrttree.Add(Points[Points.Count() - 4 + i], new GH_Path(i));
      int listitem = 0;
    ...
    Brep ss = Brep.CreateFromCornerPoints(Points[0], Points[1], Points[2], Points[3], 0);
    Brep ts = Brep.CreateFromCornerPoints(Points[pnum - 4], Points[pnum - 3], Points[pnum - 2], Points[pnum - 1], 0);
    ...
    Point3d p1 = new Point3d((Points[0].X + Points[1].X + Points[2].X + Points[3].X) / 4, (Points[0].Y + Points[1].Y + Points[2].Y + Points[3].Y) / 4, (Points[0].Z + Points[1].Z + Points[2].Z + Points[3].Z) / 4);
    Point3d p2 = new Point3d((Points[pnum - 4].X + Points[pnum - 3].X + Points[pnum - 2].X + Points[pnum - 1].X) / 4, (Points[pnum - 4].Y + Points[pnum - 3].Y + Points[pnum - 2].Y + Points[pnum - 3].Y) / 4, (Points[pnum - 4].Z + Points[pnum - 3].Z + Points[pnum - 2].Z + Points[pnum - 1].Z) / 4);
    ...
    //meshtree.Add((U + 1) * (V + 1), pth); // number of nodes per division
    meshtree.Add(U * V, pth); //number of surfaces per division
  ...
  object Orientation = null;
    ...
    //4. Invoke RunScript
    RunScript(Points, Degree, Divisions, U, V, ref Mesh, ref Srf, ref SourceSrf, ref TargetSrf, ref Orientation);
